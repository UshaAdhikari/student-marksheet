			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="marks" title="${message(code: 'marksheet.obtainedMarks.label', default: 'Obtained Marks')}" />

						<th><g:message code="marksheet.totalMarks.label" default="Total Marks" /></th>
				...
				<g:each in="${marksheetInstanceList}" status="i" var="marksheetInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${marksheetInstance.obtainedMarks}</td>

						<td>${marksheetInstance.sub.totalMarks}</td>

						%{--<td>${fieldValue(bean: marksheetInstance, field: "obtainedMarks")}</td>--}%
					
						%{--<td><g:link action="show" id="${marksheetInstance.id}">${fieldValue(bean: marksheetInstance, field: "obtainedMarks")}</g:link></td>--}%
